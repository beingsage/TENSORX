...
module BP3D.Items {
  ...
  export abstract class Item extends THREE.Mesh {
    ...
    public resize(height: number, width: number, depth: number) {
      const curW = this.getWidth();
      const curH = this.getHeight();
      const curD = this.getDepth();
    ...
    public setScale(x: number, y: number, z: number) {
      const clamp = (v: number) => {
        if (!isFinite(v) || v === 0) return 0.0001;
        const sign = v < 0 ? -1 : 1;
        let abs = Math.abs(v);
        if (abs < 0.0001) abs = 0.0001;
        return sign * abs;
      };
      x = clamp(x);
      y = clamp(y);
    ...
    public updateHighlight() {
      ...
      const matAny: any = this.material as any;
      const mats: any[] = matAny && matAny.materials ? matAny.materials : (matAny ? [matAny] : []);
      mats.forEach((material) => {
        // Not all materials expose emissive in r69
